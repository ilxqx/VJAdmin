+function ($) {
    ...
    var BJUI = {
        ...
        pageInfo: {pageCurrent:'page_current', pageSize:'page_size', orderField:'order_field', orderDirection:'order_direction'},
        alertMsg: {displayPosition:'topcenter', alertTimeout: 30000}, //alertmsg display position && close timeout
        ajaxTimeout: 30000,
        statusCode: {ok: 1, error: 0, timeout: 2},
...
+function ($) {
    ...
    $(function() {
        ...
        /* 框架名称 */
        BJUI.setRegional('uititle', 'VJUI')
        
        /* 主navtab标题 */
        BJUI.setRegional('maintab', '主页')
...
+function ($) {
    ...
    window.FRAG = {
        dialog: '<div class="bjui-dialog bjui-dialog-container" style="top:150px; left:300px;">' +
                ...
                '        <h1><span><i class="fa fa-#icon#"></i></span> <span class="title">#title#</span></h1>' +
                '    </div>' +
        ...
        gridPaging: '<ul class="pagination">' +
                    ...
                    '        <a href="javascript:;"><i class="fa fa-step-backward"></i> #first#</a>' +
                    '    </li>' +
                    '    <li class="page-prev btn-nav">' +
                    '        <a href="javascript:;"><i class="fa fa-backward"></i> #prev#</a>' +
                    '    </li>' +
                    '    #pageNumFrag#' +
                    '    <li class="page-next btn-nav">' +
                    '        <a href="javascript:;"><i class="fa fa-forward"></i> #next#</a>' +
                    '    </li>' +
                    '    <li class="page-last btn-nav">' +
                    '        <a href="javascript:;"><i class="fa fa-step-forward"></i> #last#</a>' +
                    '    </li>' +
                    '</ul>'
        ,
    ...
    Sidenav.prototype.ajaxHnav = function() {
        ...
        var createA = function(data) {
            ...
            if (data.url && (target === 'dialog' || target === 'navtab')) {
                ...
                if (data.icon)   liHtml.add(', icon:\''+ data.icon + '\'')
        ...
        var createChild = function(data) {
            ...
            if (data[childKey]) {
                ...
                $.each(data[childKey], function(i, m) {
                    var target = m[targetKey] || 'navtab', id = m[idKey] && String(m.id).isNormalID && m.id
        ...
        var okCallback = function(json) {
            ...
            $.each(json, function(i, n) {
                var target = n[targetKey] || 'navtab', id = n[idKey] && String(n[idKey]).isNormalID && n[idKey]
    ...
    Sidenav.prototype.initHnav = function() {
        ...
        var createChild = function(data) {
            ...
            if (data[childKey]) {
                ...
                $.each(data[childKey], function(i, m) {
                    var target = m[targetKey] || 'navtab', id = m[idKey] && String(m[idKey]).isNormalID && m[idKey]
...
+function ($) {
    ...
    $(function() {
        var INIT_NAVTAB = function() {
            ...
            $moreBtn.click(function() {
                ...
                var hh = $(window).height(), mh = $moreBox.height(), th = $('#bjui-top').height() + $('#navbar').height(), sh = hh - th - 26
    ...
    Navtab.prototype.openTab = function() {
        ...
        if (iOpenIndex >= 0) {
            ...
            var tabFrag = '<li><a href="javascript:" title="#title#"><span><i class="fa fa-#icon#"></i> #title#</span></a><span class="close">&times;</span></li>',
                $tab = $(tabFrag.replaceAll('#title#', options.title).replace('#icon#', options.icon)),
                $panel = $('<div class="navtabPage unitBox"></div>'),
                $more  = $('<li><a href="javascript:" title="#title#"><i class="fa fa-#icon#"></i> #title#</a></li>'.replaceAll('#title#', options.title).replace('#icon#', options.icon))
...
+function ($) {
    ...
    Dialog.prototype.open = function() {
        ...
        if ($dialog) { //if the dialog id already exists
            ...
            else if (options.minable && $.fn.taskbar) this.$element.taskbar({id:options.id, title:options.title, icon:options.icon})
...
+function ($) {
    ...
    Taskbar.prototype.init = function() {
        ...
        this.show()
        if (!$task.length) {
            var taskFrag = '<li id="#taskid#"><div class="taskbutton"><span><i class="fa fa-#taskicon#"></i></span> <span class="title">#title#</span></div><div class="close"><i class="fa fa-times-circle"></i></div></li>';
            
            $task = $(taskFrag.replace('#taskid#', 'task-'+ this.options.id).replace('#title#', this.options.title).replace('#taskicon#', this.options.icon))
...
+function ($) {
    ...
    Pagination.DEFAULTS = {
        width          : '100%',
        pageSize       : 20,
        selectPageSize : '20, 40, 60, 80, 100',
        pageCurrent    : 1,
        total          : 0,
        showPagenum    : 5,
        form           : null,
    ...
    Pagination.prototype.init = function() {
        ...
        pagingHtml
            ...
            .add('<span></span><div class="paging-pagesize"><button type="button" class="btn-default btn-refresh" title="'+ pr.refresh +'" data-icon="refresh">刷新</button>')
            .add('<select data-toggle="selectpicker" data-width="80"></select>')
            .add('</div>')
...
+function ($) {
    ...
    $(function() {
        (function() {
            $('body').theme('setFont', 'bjui-font-c')
        })()
    })
...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        /* neditor */
        if (UE) {
            $box.find('textarea[data-toggle="neditor"]').each(function() {
        ...
        * boostrap文件上传集成
        * */
        var $uploads = $box.find('[data-toggle="fileinput"]')
        $uploads.each(function () {
            var $element = $(this)
            var options  = $element.data()
            var preview = [];
            options.maxSize = options.maxSize && Number(options.maxSize) || 2048
            options.allowExt = options.allowExt && options.allowExt.split(/\s*,\s*/) || []
            options.preview && preview.push(options.preview)
            $element.fileinput({
                'theme': 'fa',
                'language': 'zh',
                'showUpload': false,
